char AES_KEY[16] = {0xbf, 0x98, 0xcd, 0xb8, 0xf1, 0x81, 0xb8, 0x44, 0x75, 0x5d, 0x0c, 0xce, 0xd1, 0x23, 0xbf, 0x15};
// Hardware Imports
...
void load_firmware(void) {
  ...
  while (true) {
    ...
    // Get chunk size.
    rcv         = uart_read(UART1, BLOCKING, &read);
    chunk_size  = (uint32_t)rcv;
    rcv         = uart_read(UART1, BLOCKING, &read);
    chunk_size |= (uint32_t)rcv << 8;
    
    uart_write_str(UART2, "Received Chunk Size: ");
    uart_write_hex(UART2, chunk_size);
    nl(UART2);
